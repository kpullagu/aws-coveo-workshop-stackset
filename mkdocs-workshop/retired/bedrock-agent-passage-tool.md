...
- ✅ Verify that conversation memory is already enabled in AWS
...
- ✅ Review Memory configuration for multi-turn conversations (to be tested in Lab 4)
...
!!! info "Bedrock Agent memory is pre-configured"
    In this lab you will review a Bedrock Agent that already has session-summary memory configured through CloudFormation. The workshop UI still uses the single-turn search experience here; you will test multi-turn and cross-session behavior in Lab 4.
...
2. **Memory**: You have access to conversation history within the current session, and session summaries from previous sessions for the same memory ID after those sessions end or time out.
...
**Important:** You can always recall conversations within the current session. Previous-session recall is available only after the earlier session has ended or timed out and a summary has been stored.
...
- "What did we talk about last time?" → If memory is available: "In our previous session, we discussed Roth IRA benefits." If no memory is available yet: "I don't have access to our previous conversations yet, but I'm happy to help with any questions you have now."
