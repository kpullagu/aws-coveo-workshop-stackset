...
| **Session + Cross-Session Memory** | Multiple sessions | Across logins | AgentCore (Coveo Hosted MCP) |
...
| **Memory ID Source** | Cognito `sub` (hashed) | Cognito `sub` (hashed) |
...
|---------|-------|-------------|
| **Memory Type** | Session + Cross-session | Both types enabled |
| **Actor ID Source** | Cognito `sub` (raw) | Cognito `sub` (raw) |
...
!!! success "What You Learned"
    ...
    - Memory persists across logout/login cycles when the same user identity is used
    - Both Bedrock Agent and AgentCore support cross-session recall, but the prior session must end or time out before summaries are available
    ...
    - Testing cross-session memory requires ending the active session first; logout/login is optional
    - Memory ID or Actor ID is the key to enabling cross-session persistence
