...
# Usage: bash scripts/package-lambdas.sh <bucket-name> [region]
...
if [ -z "$BUCKET_NAME" ]; then
    ...
    echo "Usage: bash scripts/package-lambdas.sh <bucket-name> [region]"
    exit 1
fi
...
for FUNC_NAME in "${LAMBDA_FUNCTIONS[@]}"; do
    ...
    cd "$SOURCE_DIR"
    zip -q -r "$ZIP_FILE" . -x "*.pyc" -x "__pycache__/*" -x ".DS_Store"
    cd "$PROJECT_ROOT"
